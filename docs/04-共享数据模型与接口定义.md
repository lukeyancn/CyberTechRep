# 共享数据模型与模块接口定义（ClassIng.Shared）

> 项目：ClassIng（ClassIsland QQ 群消息接管与课堂信息分发系统）
> 模块：模块 0 最终交付物之一。本文档是后续模块 1~10 的**统一契约**：所有模块只依赖 `ClassIng.Shared`（接口 + DTO），禁止跨模块引用实现类。
...
> 代码实现以 `src/ClassIng.Shared` 为准，本文档字段类型若与代码冲突，以代码为准。
...
| 模块间只经接口通信 | 全部接口集中于 `ClassIng.Shared/Abstractions`，实现各自封闭 | ✅ |
