...
flowchart TB
    ...
    subgraph ClassIsland宿主进程["ClassIsland 宿主进程（CyberTechRep 插件）"]
        direction TB
        ...
        subgraph UI["表现层（模块6/7/8/9）"]
            ...
            SP[设置页<br/>CyberTechRepSettingsPage]
...
| 模块 | 名称 | 职责 | 依赖 |
| --- | --- | --- | --- |
| 模块 1 | QQ 消息接入层 | OneBot 11 WebSocket 客户端、心跳、指数退避重连、`message_id` 幂等去重、断线缓存（重试队列） | CyberTechRep.Shared（契约）；NapCat（外部） |
...
规则：
1. **所有模块只依赖 `CyberTechRep.Shared` 契约（接口+DTO），模块间不直接引用实现类**，由 DI 容器组装；
...
**结论：采用「单插件项目（CyberTechRep.Plugin）+ 独立共享契约项目（CyberTechRep.Shared）」共两个项目的解决方案；插件内部按文件夹分层。** 不拆分为主插件+多个子项目。
...
3. `CyberTechRep.Shared` 单独成项目的唯一目的：**契约与宿主/UI/JSON 实现解耦**——它不引用 Avalonia、不引用 ClassIsland.Core，可被未来的独立接入器进程、单元测试工程直接复用，也能独立编译验证；
...
├─ docs\                          # 本模块交付文档
├─ src\
│  ├─ CyberTechRep.Shared\            # 契约层（纯 net8.0 类库，无 Avalonia/宿主依赖）
│  │  ├─ Models\                  # DTO：MessageRecord / ClassifiedMessage / NoticeItem / ...
│  │  ├─ Abstractions\            # 接口：IMessageIngestService / IMessageClassifier / ...
│  │  └─ CyberTechRep.Shared.csproj
│  └─ CyberTechRep.Plugin\            # 模块 1~9 实现（后续模块创建，本模块不实现）
│     ├─ Services\                # 接入/分类/识别/文件/存储/重试 实现
│     ├─ Views\                   # 悬浮窗、排错面板（Avalonia）
│     ├─ Controls\                # 设置页与附属控件
│     ├─ manifest.yml
│     └─ CyberTechRep.Plugin.csproj   # 引用 ClassIsland.PluginSdk，CreateCipx=true
├─ ClassIsland\                   # 上游源码（只读参考，不修改）
└─ CyberTechRep.sln                   # 解决方案
```
