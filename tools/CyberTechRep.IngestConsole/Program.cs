...
namespace CyberTechRep.IngestConsole;

/// <summary>
/// 模块 1 验收自检工具：连接真实 QQ 官方机器人平台，把收到的群消息打印为结构化日志。
/// 用法：
///   dotnet run --project tools/CyberTechRep.IngestConsole -- --appid 123456 --secret YOUR_SECRET [--groupopenid AACDxxx]
/// 或用环境变量：CYBERTECHREP_APPID / CYBERTECHREP_SECRET。
/// </summary>
internal static class Program
{
    private static async Task<int> Main(string[] args)
    {
        ...
        appId ??= Environment.GetEnvironmentVariable("CYBERTECHREP_APPID");
        secret ??= Environment.GetEnvironmentVariable("CYBERTECHREP_SECRET");

        if (string.IsNullOrEmpty(appId) || string.IsNullOrEmpty(secret))
        {
            Console.WriteLine("缺少凭据。用法：");
            Console.WriteLine("  dotnet run --project tools/CyberTechRep.IngestConsole -- --appid <AppID> --secret <AppSecret> [--groupopenid <群OpenID>]");
            return 1;
        }
        ...
        var logger = loggerFactory.CreateLogger("CyberTechRep.IngestConsole");
